...
public class ShipObject
{
    ...
    public ShipObject bank(int distance, int LR, int numSteps)
    {
        ...
        if(base == 4 && distance == 1) {
            ...
            l_totalDist = 21.72;
        } else if(base == 6 && distance == 1) {
            l_totalDist = 11.97;
        } else if(base == 6 && distance == 2) {
            l_totalDist = 15.90;
        } else if(base == 6 && distance == 3) {
            l_totalDist = 19.83;
        }
    ...
    public ShipObject barrelRoll(int LR, int numSteps, int BEF)
    {
        if(numSteps != 100) {
            ShipObject testShip = new ShipObject("Test", xLoc, yLoc, bearing, base, team);
            return testShip;
        }
        ...
        double l_testY = yLoc;
        double l_bearing = bearing;
        double l_totalDist = (double) (4 + base);
        if(LR == 0) {
            l_bearing = l_bearing - 90.0;
        } else if (LR == 1){
            l_bearing = l_bearing + 90.0;
        }
        ...
        if(base == 4) {
            ...
            l_testX = l_testX + (double) (2+base) * (double) (Math.sin(d2R(l_bearing)));
            l_testY = l_testY + (double) (2+base) * (double) (Math.cos(d2R(l_bearing)));
        }
        ...
        if (BEF == -1) {
            l_testX = l_testX + (double) ((base - 2) * -1) * (double) (Math.sin(d2R(l_bearing)));
            l_testY = l_testY + (double) ((base - 2) * -1) * (double) (Math.cos(d2R(l_bearing)));
        }
        if (BEF == 1) {
            l_testX = l_testX + (double) (base - 2) * (double) (Math.sin(d2R(l_bearing)));
            l_testY = l_testY + (double) (base - 2) * (double) (Math.cos(d2R(l_bearing)));
        }
    ...
    public ShipObject uturn(int distance, int numSteps)
    {
        double l_testX = xLoc;
        double l_testY = yLoc;
        double l_bearing = bearing;
    ...
    public ShipObject rotate(int lr)
    {
        double l_bearing = 0;
        if(lr == 0)
            l_bearing = bearing - 90;
        if(lr == 1)
            l_bearing = bearing +90;

        ShipObject testShip = new ShipObject("Test", xLoc, yLoc, l_bearing, base, team);
        return  testShip;

    }

    public ShipObject bump(int fr, int numSteps)
    {
        if(numSteps != 100) {
            ShipObject testShip = new ShipObject("Test", xLoc, yLoc, bearing, base, team);
            return testShip;
        ...
        double l_bearing = bearing;
        if(fr == 0) {
            l_testX = l_testX + (double) (1) * (double) (Math.sin(d2R(l_bearing)));
            l_testY = l_testY + (double) (1) * (double) (Math.cos(d2R(l_bearing)));
        }
        if(fr == 1) {
            l_testX = l_testX + (double) (-1) * (double) (Math.sin(d2R(l_bearing)));
            l_testY = l_testY + (double) (-1) * (double) (Math.cos(d2R(l_bearing)));
